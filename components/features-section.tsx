...
export function FeaturesSection() {
  ...
  return (
    <section id="about" className="w-full py-12 md:py-20 lg:py-24 bg-gradient-to-b from-white to-blue-50">
      <div className="container px-4 md:px-6 mx-auto">
        <div className="grid gap-8 lg:grid-cols-2">
          <div className="space-y-5">
            <div className="inline-block rounded-full bg-blue-100 px-3 py-1 text-xs font-medium text-blue-700 animate-pulse">
              Why Choose Us
            </div>
            <h2 className="text-3xl font-bold tracking-tight md:text-4xl text-gray-900">
              India's Top Odoo<br />Service Provider
            </h2>
            <p className="text-base text-gray-600 md:text-lg max-w-lg"> 
            ...
            <div className="grid gap-4 md:grid-cols-2 mt-6">
              {featuresData.map((feature, index) => (
                <div 
                  key={index}
                  className="group flex flex-col p-3 rounded-xl bg-white shadow-sm hover:shadow-md hover:bg-blue-50 transition-all duration-300 border border-gray-100 hover:-translate-y-1"
                  onMouseEnter={() => setHoveredIndex(index)}
                  onMouseLeave={() => setHoveredIndex(null)}
                >
                  <div className="flex items-start gap-2 mb-1">
                    <CheckCircle2 
                      className={`h-5 w-5 text-blue-600 flex-shrink-0 mt-0.5 transition-all duration-300 ${hoveredIndex === index ? 'text-blue-700 scale-110' : ''}`} 
                    />
                    <h3 className="font-medium text-sm text-gray-900">{feature.title}</h3>
                  </div>
                  <p className="text-xs text-gray-600 pl-7">{feature.description}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="relative flex items-center justify-center">
            <div className="absolute w-4/5 h-4/5 bg-blue-600 rounded-full opacity-5 blur-3xl animate-pulse"></div>
